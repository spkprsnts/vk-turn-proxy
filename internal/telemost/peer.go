...
func (p *Peer) handleSignaling(ctx context.Context) {
	...
	for {
		...
		if err := p.ws.ReadJSON(&msg); err != nil {
			debugf("Telemost ws read error: %v", err)
			if !p.closed.Load() {
				p.queueReconnect()
			}
			return
		...
		if offer, ok := msg["subscriberSdpOffer"].(map[string]interface{}); ok && !pubSent {
			if err := p.handleSdpOffer(offer, uid); err != nil {
				debugf("Telemost sdp offer error: %v", err)
				continue
			}
			pubSent = true
		}
...
func (p *Peer) handleSdpOffer(offer map[string]interface{}, uid string) error {
	...
	if p.onData == nil {
		if err := p.sendSetSlots(); err != nil {
			debugf("Telemost setSlots error: %v", err)
		}
	}
...
func (p *Peer) sendTelemetry(ctx context.Context, endpoint, event string) {
	...
	if err != nil {
		debugf("Telemost telemetry req error: %v", err)
		return
	}
	...
	if err != nil {
		debugf("Telemost telemetry send error: %v", err)
		return
	}
...
func (p *Peer) Close() error {
	...
	p.closeOnce.Do(func() {
		log.Printf("Telemost peer Close: sending leave")
		leaveUID := uuid.New().String()
		leaveAck := p.registerAckWaiter(leaveUID)
		if p.sendLeave(leaveUID) {
			log.Printf("Telemost peer Close: leave sent, waiting for ack")
			_ = p.waitForAck(leaveUID, leaveAck, 1500*time.Millisecond)
			log.Printf("Telemost peer Close: ack done")
		} else {
			log.Printf("Telemost peer Close: sendLeave failed (ws nil or closed)")
...
func (p *Peer) sendWSPing() bool {
	...
	if p.ws != nil {
		if err := p.ws.WriteControl(websocket.PingMessage, []byte{}, time.Now().Add(10*time.Second)); err != nil {
			debugf("Telemost ws ping error: %v", err)
			p.queueReconnect()
			return false
		}
	}
...
func (p *Peer) sendAppPing() bool {
	...
	if p.ws != nil {
		if err := p.ws.WriteJSON(map[string]interface{}{
			...
			debugf("Telemost app ping error: %v", err)
			p.queueReconnect()
			return false
		}
	}
...
func (p *Peer) retryReconnect(ctx context.Context, backoff time.Duration) bool {
	for {
		if err := p.reconnect(ctx); err != nil {
			debugf("Telemost reconnect failed: %v", err)
			select {
			case <-ctx.Done():
				return true
			case <-p.closeCh:
	...
	for {
		...
		case data := <-p.sendQueue:
			if len(data) > p.trafficShape.MaxMessageSize {
				debugf("Telemost oversized message size=%d limit=%d", len(data), p.trafficShape.MaxMessageSize)
				continue
			}
			...
			if waited > 0 {
				debugf("[WORKER-%d] Drained after %v", workerID, waited)
			}

			if err := p.dc.Send(data); err != nil {
				debugf("Telemost send error: %v", err)
				p.queueReconnect()
				return
			}
	...
	for p.dc.BufferedAmount() > 512*1024 {
		...
		case <-time.After(10 * time.Millisecond):
			if time.Since(start) > 5*time.Second {
				debugf("Telemost buffer wait timeout worker=%d", workerID)
	...
	for {
		...
		case <-ticker.C:
			...
			if queueLen > 100 || buffered > 1024*1024 {
				log.Printf("Telemost queue=%d buf=%dMB", queueLen, buffered/(1024*1024))
			}
		}
	}
}
...
var ErrPublisherNotInitialized = errors.New("publisher peer connection not initialized")
